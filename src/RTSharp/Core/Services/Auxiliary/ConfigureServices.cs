...
namespace RTSharp.Core.Services.Auxiliary
{
    public static class ConfigureServices
    {
        ...
        public static async Task GenerateCertificatesIfNeeded()
        {
            if (!Path.Exists(CertPath) || !Path.Exists(KeyPath)) {
                var res = await Dispatcher.UIThread.InvokeAsync(async () => {
                    ...
                    "RT#",
                    "No client certificate exists for auxiliary service, do you wish to generate a new one?",
                    ButtonEnum.YesNo,
                    Icon.Question,
        ...
        public static void AddAuxiliaryServices(this IServiceCollection services, Dictionary<string, Server> servers)
        {
            ...
            void register<T>(string name, Server server)
                where T : class
            {
                services
...
```",
            ...
            if (servers != null) {
                foreach (var server in servers) {
                    register<RTSharp.Auxiliary.Protocols.GRPCServerService.GRPCServerServiceClient>(nameof(RTSharp.Auxiliary.Protocols.GRPCServerService.GRPCServerServiceClient) + "_" + server.Key, server.Value);
                    register<RTSharp.Auxiliary.Protocols.GRPCFilesService.GRPCFilesServiceClient>(nameof(RTSharp.Auxiliary.Protocols.GRPCFilesService.GRPCFilesServiceClient) + "_" + server.Key, server.Value);
                }
            }
        }
    }
