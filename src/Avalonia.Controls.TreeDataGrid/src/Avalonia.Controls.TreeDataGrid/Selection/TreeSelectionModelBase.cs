...
namespace Avalonia.Controls.Selection
{
    public abstract class TreeSelectionModelBase<T> : ITreeSelectionModel, INotifyPropertyChanged
    {
        ...
        internal void OnNodeSelectedItemReplaced(object sender, int index, IList oldItems, IList newItems)
        {
            TreeSelectionNode<T> lookupObj(TreeSelectionNode<T> inp)
            {
                if (inp.Source?.Equals(sender) == true)
                {
                    return inp;
                }
                if (inp.Children != null)
                {
                    foreach (var c in inp.Children.Where(x => x != null))
                    {
                        return lookupObj(c!);
                    }
                }
            ...
            var replaceNode = lookupObj(_root);
        
            var firstElementMatches = Object.Equals(newItems[0], oldItems[0]);
        
            if (replaceNode.Path.Append(index) == _selectedIndex && !firstElementMatches)
            {
                RaisePropertyChanged(nameof(SelectedItem));
            }
        
            bool replacementMatches = true;
            for (var i = 1;i < newItems.Count; i++)
            {
                if (!Object.Equals(oldItems[i], newItems[i]))
                {
            ...
            if (!replacementMatches || !firstElementMatches)
            {
                var e = new TreeSelectionModelSelectionChangedEventArgs<T>(
                    deselectedItems: oldItems.Cast<T?>().ToList(),
                    selectedItems: newItems.Cast<T?>().ToList()
                );
                SelectionChanged?.Invoke(this, e);
                _untypedSelectionChanged?.Invoke(this, e);
            }
