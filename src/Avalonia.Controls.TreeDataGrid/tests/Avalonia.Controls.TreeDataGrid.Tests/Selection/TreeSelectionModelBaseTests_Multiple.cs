        ...
        public class CollectionChanges
        {
            ...
            public void Replacing_Selected_Root_Item_Updates_State()
            {
                ...
                target.SelectionChanged += (s, e) =>
                {
                    ...
                    Assert.Equal(new[] { "Node 1" }, e.DeselectedItems.Select(x => x!.Caption));
                    Assert.Empty(e.SelectedIndexes);
                    Assert.Equal(new[] { "new" }, e.SelectedItems.Select(x => x!.Caption));
                    ++selectionChangedRaised;
                };
                ...
                Assert.Equal(1, target.Count);
                Assert.Equal(new IndexPath(1), target.SelectedIndex);
                Assert.Equal([ new IndexPath(1) ], target.SelectedIndexes);
                Assert.Equal("new", target.SelectedItem!.Caption);
                Assert.Equal([ "new" ], target.SelectedItems.Select(x => x!.Caption));
                Assert.Equal(1, selectionChangedRaised);
            ...
            public void Replacing_Selected_Child_Item_Updates_State()
            {
                ...
                Assert.Equal(1, target.Count);
                Assert.Equal(new IndexPath(1, 1), target.SelectedIndex);
                Assert.Equal([ new IndexPath(1, 1) ], target.SelectedIndexes);
                Assert.Equal("new", target.SelectedItem!.Caption);
                Assert.Equal([ "new" ], target.SelectedItems.Select(x => x!.Caption));
                Assert.Equal(1, selectionChangedRaised);
                Assert.Equal(0, selectedIndexRaised);
            ...
            [AvaloniaFact(Timeout = 10000)]
            public void Replacing_Root_Item_Doesnt_Select()
            {
                var data = CreateData();
                var target = CreateTarget(data);
                var raised = 0;
            ...
            [AvaloniaFact(Timeout = 10000)]
            public void Replacing_Child_Item_Doesnt_Select()
            {
                var data = CreateData();
                var target = CreateTarget(data);
                var raised = 0;
