    ...
    public class HierarchicalTreeDataGridSourceTests
    {
        public class RowsAndCells
        {
            ...
            public void Supports_Replacing_Root_Row(bool sorted)
            {
                ...
                target.RowSelection!.Select(new IndexPath(2));
                ...
                // Rows collection changed, first removed then added
            ...
            [InlineData(false)]
            [InlineData(true)]
            public void Supports_Replacing_Root_Row_With_Selection(bool sorted)
            {
                var data = CreateData();
                var target = CreateTarget(data, sorted);
                ...
                var raised = 0;
                target.RowSelection!.SelectionChanged += (s, e) => ++raised;
            ...
            [InlineData(false)]
            [InlineData(true)]
            public void Supports_Replacing_Selected_Root_Row(bool sorted)
            {
                var data = CreateData();
                var target = CreateTarget(data, sorted);
                ...
                // Rows collection changed, first removed then
                Assert.Equal(2, raised);
            }
