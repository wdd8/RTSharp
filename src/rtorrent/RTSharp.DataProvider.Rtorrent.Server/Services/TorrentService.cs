...
namespace RTSharp.DataProvider.Rtorrent.Server.Services
{
	public class TorrentService
	{
		...
		public TorrentService(SCGICommunication Scgi)
		{
			this.Scgi = Scgi;
		}
		...
		public async Task<InfoHashDictionary<(string BasePath, string? ContainerFolder)>> GetTorrentsBasePath(IEnumerable<byte[]> Hashes)
		{
			...
			foreach (var hash in Hashes) {
				if (XMLUtils.GetValueType(result) == SCGI_DATA_TYPE.STRUCT) {
					...
					throw new RpcException(new Grpc.Core.Status(StatusCode.Internal, $"Failed to get name for {Convert.ToHexString(hash)}. " + status.ToFailureString()));
				}
				...
				if (XMLUtils.GetValueType(result) == SCGI_DATA_TYPE.STRUCT) {
					...
					throw new RpcException(new Grpc.Core.Status(StatusCode.Internal, $"Failed to get is_multi_file for {Convert.ToHexString(hash)}. " + status.ToFailureString()));
				}
