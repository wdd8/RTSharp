...
using Status = RTSharp.DataProvider.Rtorrent.Protocols.Types.Status;
    ...
    public partial class TorrentService : GRPCTorrentService.GRPCTorrentServiceBase
    {
            ...
            foreach (var (id, torrent) in torrents) {
                ...
                try {
                    ...
                    ret.Torrents.Add(new TorrentsReply.Types.TorrentReply() {
                        ...
                        Status = { new Protocols.Types.Status() {
                            Command = "",
                            FaultCode = "1",
                ...
                ret.Torrents.Add(new TorrentsReply.Types.TorrentReply() {
                    ...
                    Status = { Extensions.SuccessfulStatus("load_raw") }
                });
            }
        ...
        public override async Task<TorrentsPeersReply> GetTorrentsPeers(Torrents Req, ServerCallContext Ctx)
        {
            ...
            if ((fault = XMLUtils.TryGetFaultStruct(ref result, "p.multicall")) != null) {
                throw new RpcException(new Grpc.Core.Status(StatusCode.Internal, fault.FaultCode + ": " + fault.FaultString));
            }
        ...
        public override async Task MoveDownloadDirectory(MoveDownloadDirectoryArgs Req, IServerStreamWriter<MoveDownloadDirectoryProgress> Res, ServerCallContext Ctx)
        {
            ...
            try {
                var proc = await Cli.Wrap("rsync").WithArguments("--version").WithValidation(CommandResultValidation.None).ExecuteBufferedAsync(Ctx.CancellationToken);
                if (proc.ExitCode != 0) {
                    throw new RpcException(new Grpc.Core.Status(StatusCode.FailedPrecondition, "Rsync is not installed. ExitCode " + proc.ExitCode));
                }
            } catch (Exception ex) {
                Logger.LogError(ex, "Rsync installation check error");
                throw new RpcException(new Grpc.Core.Status(StatusCode.FailedPrecondition, "Rsync is not installed"));
            }
            ...
            /* Consistency check */ {
                ...
                foreach (var torrent in files.Reply) {
                    if (!basePaths.TryGetValue(torrent.InfoHash.ToByteArray(), out var basePath))
                        throw new RpcException(new Grpc.Core.Status(StatusCode.Internal, $"Failed to get base directory for {Convert.ToHexString(torrent.InfoHash.ToByteArray())}, terminating operation"));
                ...
                if (!checkA.SequenceEqual(checkB)) {
                    throw new RpcException(new Grpc.Core.Status(StatusCode.Unavailable, "Client-side and server-side did not agree on identical list of files"));
                }
            }
