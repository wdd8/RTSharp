...
{
    public partial class Grpc(QbitClient Client)
    {
        private CancellationTokenSource TorrentChangesTokenSource;
        private InfoHashDictionary<TorrentInfo> LatestTorrents = new();
        ...
        public async Task<IList<(byte[] Hash, IList<Exception> Exceptions)>> ForceRecheck(IList<byte[]> In)
        {
            ...
            try {
                await Client.Client.RecheckAsync(In.Select(Convert.ToHexString));
            } catch (Exception ex) {
                exception = ex;
            }
            ...
            _ = Task.Run(async () => {
                ...
                try {
                    ...
                    while (!TorrentChangesTokenSource.Token.IsCancellationRequested && !Active.IsCancellationRequested) {
                        ...
                        var partialData = await Client.Client.GetPartialDataAsync(rid, TorrentChangesTokenSource.Token);
        ...
        public async Task<IList<(byte[] Hash, IList<Exception> Exceptions)>> MoveDownloadDirectory(IList<(byte[] InfoHash, string TargetDirectory)> In, IList<(string SourceFile, string TargetFile)> Check, IProgress<(byte[] InfoHash, string File, ulong Moved, string? AdditionalProgress)> Progress)
        {
            ...
            foreach (var req in In) {
                tasks.Add(Task.Run(async () => {
                    ...
                    try {
                        await Client.Client.SetLocationAsync(Convert.ToHexString(req.InfoHash), req.TargetDirectory);
                    } catch (Exception ex) {
                        exception = ex;
                    }
        ...
        public async Task<IList<(byte[] Hash, IList<Exception> Exceptions)>> SetLabels(IList<(byte[] Hash, string[] Labels)> In)
        {
            ...
            foreach (var (hash, labels) in In) {
                tasks.Add(Task.Run(async () => {
                    ...
                    try {
                        await Client.Client.ClearTorrentTagsAsync(Convert.ToHexString(hash));
                        await Client.Client.AddTorrentTagsAsync(Convert.ToHexString(hash), labels);
                    } catch (Exception ex) {
                        exception = ex;
                    }
